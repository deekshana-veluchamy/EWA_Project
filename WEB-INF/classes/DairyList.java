...
@WebServlet("/DairyList")

public class DairyList extends HttpServlet {

	/* Dairy Page Displays all the dairies and their Information in Game Speed */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
		   if(CategoryName.equals("milkyogurt"))
		   {
			 for(Map.Entry<String,Dairy> entry : SaxParserDataStore.dairies.entrySet())
			 {
				if(entry.getValue().getRetailer().equals("Yogurts"))
				 {
		   ...
		   else if(CategoryName.equals("eggs"))
		    {
				for(Map.Entry<String,Dairy> entry : SaxParserDataStore.dairies.entrySet())
					{
					 if(entry.getValue().getRetailer().equals("Eggs"))
					 {
						 hm.put(entry.getValue().getId(),entry.getValue());
					 }
					}
			...
			else if(CategoryName.equals("cookiesdoughs"))
			{
				for(Map.Entry<String,Dairy> entry : SaxParserDataStore.dairies.entrySet())
				{
				 if(entry.getValue().getRetailer().equals("Doughs"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
			...
			else if(CategoryName.equals("desserts"))
			{
				for(Map.Entry<String,Dairy> entry : SaxParserDataStore.dairies.entrySet())
				{
				 if(entry.getValue().getRetailer().equals("Desserts"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
			...
			else if(CategoryName.equals("breads"))
			{
				for(Map.Entry<String,Dairy> entry : SaxParserDataStore.dairies.entrySet())
				{
				 if(entry.getValue().getRetailer().equals("Buns"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
		...
		All the Dairy and Dairy information are dispalyed in the Content Section
		...
		for(Map.Entry<String, Dairy> entry : hm.entrySet())
		{
			Dairy dairy = entry.getValue();
			if(i%3==1) pw.print("<tr>");
			pw.print("<td><div id='shop_item'>");
			pw.print("<h3>"+dairy.getName()+"</h3>");
			pw.print("<strong>$"+dairy.getPrice()+"</strong><ul>");
			pw.print("<strong>Discount:" + dairy.getDiscount() + "%</strong><ul>");
			pw.print("<li id='item'><img src='images/dairies/"+dairy.getImage()+"' alt='' /></li>");

			pw.print("<li><form method='post' action='Cart'>" +
					"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
					"<input type='hidden' name='type' value='dairies'>"+
					"<input type='hidden' name='maker' value='"+dairy.getRetailer()+"'>"+
					"<input type='hidden' name='access' value=''>"+
					"<input type='submit' class='btnbuy' value='Buy Now'></form></li>");
			pw.print("<li><form method='post' action='WriteReview'>"+"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
					"<input type='hidden' name='type' value='dairies'>"+
					"<input type='hidden' name='maker' value='"+dairy.getRetailer()+"'>"+
					"<input type='hidden' name='price' value='"+dairy.getPrice()+"'>"+
					"<input type='hidden' name='access' value=''>"+
			...
			pw.print("<li><form method='post' action='ViewReview'>"+"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
				    ...
				    "<input type='submit' value='ViewReview' class='btnreview'></form></li>");
						pw.print("<li><form method='post' action='Favourite'>"+"<input type='hidden' name='name' value='"+entry.getKey()+"'>"+
								"<input type='hidden' name='type' value='DairyList'>"+
								"<input type='hidden' name='id' value='"+dairy.getId()+"'>"+
								"<input type='hidden' name='maker' value='"+dairy.getRetailer()+"'>"+
								"<input type='hidden' name='discount' value='"+dairy.getDiscount()+"'>"+
								"<input type='hidden' name='price' value='"+dairy.getPrice()+"'>"+

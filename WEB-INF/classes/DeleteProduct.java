...
public class DeleteProduct extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		displaydeleteproduct(request, response, pw, false);
    }


    protected void displaydeleteproduct(HttpServletRequest request,
    ...
    pw.print("<form method='post' action='DeleteProductCode' enctype='multipart/form-data'>"
                + "<div class='container' style='margin-top: 20px;'>"


                + "<div class='panel panel-default'>"
                + "<div class='panel-heading panel-heading-green' style='background: #22C125;color: whitesmoke;'>DELETE PRODUCT</div>"
                + "</div>"

                + "<div class='form-horizontal'>"
                + "<div class='col-sm-10'>"
                + "<div class='form-group'>"
                + "<label ID='Label1' CssClass='control-label col-md-2'>ID<label>"
                + "<div class='col-md-3'>"
                + "<input type='text' name='txtpid' value='' CssClass='form-control' required></input>"
                + "</div>"
                + "</div>"
                + "<div class='form-group'>"
                + "<div class='col-md-2'></div>"
                + "<div class='col-md-3'>"
                + "<input type='submit' class='btn btn-danger'  value='DELETE PRODUCT' CssClass='col-xs-offset-0' border-style='none;' style='Width=303px; height:40px;font-size:15px; margin: 2rem; color:white; background: black;'></input>"
                + "</div>"
                + "</div>"
				+ "</div></div></form></div>");
